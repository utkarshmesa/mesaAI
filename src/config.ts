...
const EnvSchema = z.object({
  ...
  // One key, or a comma list: later keys are fallbacks on quota/rate-limit/server errors.
  GEMINI_API_KEY: z
    .string()
    .min(1)
    .transform((s) => s.split(',').map((x) => x.trim()).filter(Boolean)),
...
const ScriptSchema = EnvSchema.pick({ GEMINI_API_KEY: true, GEMINI_MODEL: true, DRAFT_MODEL: true, SCORE_THRESHOLD: true, MIN_NOTE_WORDS: true });

/** Just what eval/compare need, so they run before the Telegram bot exists. */
export function loadScriptConfig(env: Record<string, string | undefined> = process.env) {
  const parsed = ScriptSchema.safeParse(env);
